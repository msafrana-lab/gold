...
- **Continuous live FX** (`FX_API_KEY`, [Twelve Data](https://twelvedata.com), free 800/day) — configured: makes the FX leg intraday on a ~3 min cadence (paused when the tab is hidden), so the auto-refreshed CHF cross is live and the analyzer's fair value uses a live USD/CHF.
