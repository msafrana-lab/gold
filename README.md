...
- **Currency conversion**: pick USD, CHF, EUR, GBP, JPY, CNY, CAD, AUD, HKD, SGD or INR — every figure converts using an FX rate fetched from a resilient chain of free sources ([frankfurter.dev](https://frankfurter.dev) ECB → exchangerate-api → currency-api), with the rate's "as of" date shown. Falls back to USD only if every source is unreachable. For a true **intraday** rate (~60 s), drop a free [Twelve Data](https://twelvedata.com) API key into the `FX_API_KEY` constant near the top of the `<script>` and it switches automatically.
