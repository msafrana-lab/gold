...
- **Optional live feeds** (both free, key visible in page source — restrict/rotate as needed):
  - `METALS_API_KEY` — a [metals.dev](https://metals.dev) key makes the whole feed a single-source **live** call (gold + FX, ≤60 s), so CHF is a genuine live cross.
  - `FX_API_KEY` — a [Twelve Data](https://twelvedata.com) key makes just the FX leg intraday (~3 min cadence, well under the free 800/day cap). Polling pauses when the tab is hidden.
